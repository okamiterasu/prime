				...
				for (i, tracked) in self.tracked.iter().enumerate()
				{
					ui.group(|ui|
					{
						ui.vertical(|ui|
						{
							...
							ui.horizontal(|ui|
							{
								let owned = match self.owned.get_mut(&tracked.recipe.unique_name)
								{
									Some(v)=>v,
									None=>self.owned.entry(tracked.recipe.unique_name.clone()).or_insert(0)
								};
							...
							for component in &tracked.components
							{
								ui.horizontal(|ui|
								{
									let owned = match self.owned.get_mut(&component.unique_name)
									{
										Some(v)=>v,
										None=>self.owned.entry(component.unique_name.clone()).or_insert(0)
									};
